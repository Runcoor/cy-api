...
// SearchAllTopUps 按订单号搜索全平台充值记录（管理员使用），支持按状态筛选
...
// RechargeCryptomus 处理 Cryptomus 加密货币支付成功后的入账
func RechargeCryptomus(tradeNo string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if err != nil {
		common.SysError("cryptomus topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}
...
// RechargeNowPayments 处理 NowPayments 加密货币支付成功后的入账。
// 与其他 Recharge* 函数同形：行级锁防并发，状态校验幂等，事务内更新订单 + 用户额度。
func RechargeNowPayments(tradeNo string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("NowPayments 加密货币充值成功，充值额度: %v，支付金额: %.2f USD", logger.FormatQuota(quotaToAdd), topUp.Money))
		MaybeGrantInviteRewardOnFirstTopUp(topUp.UserId)
	}
...
// RechargeDodoPayments 处理 Dodo Payments 支付成功后的入账。
// 与其他 Recharge* 函数同形：行级锁防并发，状态校验保证幂等，事务里更新订单和用户额度。
func RechargeDodoPayments(tradeNo string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if err != nil {
		common.SysError("dodo payments topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}

	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("Dodo Payments 充值成功，充值额度: %v，支付金额: %.2f USD", logger.FormatQuota(quotaToAdd), topUp.Money))
		MaybeGrantInviteRewardOnFirstTopUp(topUp.UserId)
	}
