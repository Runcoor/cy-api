...
const SocialReadyPane = ({ post, t }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 760, margin: '0 auto' }}>
    ...
    {/* Publishing panel */}
    <SocialPublishPanel briefingId={post.briefing_id} t={t} />
  </div>
);
...
// Per-platform publish-status panel inside the social-post modal.
// Self-contained (own state + API calls) so it doesn't bloat the parent.
const SocialPublishPanel = ({ briefingId, t }) => {
  const [rows, setRows] = useState([]);
  const [busyOne, setBusyOne] = useState(null); // platform key currently being published
  ...
  return (
    <div>
      ...
      >
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('发布到平台')}</div>
        <div style={{ flex: 1 }} />
        <Button
          theme='solid'
          type='primary'
