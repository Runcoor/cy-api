...
export const useTaskLogsData = () => {
  ...
  // Get form values helper function — prefer controlled state
  const getFormValues = () => {
  ...
  const loadLogs = async (page = 1, size = pageSize) => {
    ...
    let url = isAdminUser
      ? `/api/task/?p=${page}&page_size=${size}&keyword=${kw}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`
      : `/api/task/self?p=${page}&page_size=${size}&keyword=${kw}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`;
