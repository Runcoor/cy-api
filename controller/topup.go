...
func GetTopUpInfo(c *gin.Context) {
	...
	// 如果启用了 NowPayments 加密货币支付，添加到支付方法列表
	enableNowPayments := nowPaymentsEnabled()
	if enableNowPayments {
		...
		if !hasNowPayments {
			nowPaymentsMethod := map[string]string{
				"name":      "NowPayments (USDT / Crypto)",
				"type":      "nowpayments",
				"color":     "rgba(var(--semi-teal-5), 1)",
				"min_topup": strconv.Itoa(setting.NowPaymentsMinTopUp),
			}
			payMethods = append(payMethods, nowPaymentsMethod)
		}
	}

	// 如果启用了 Dodo Payments 充值（含 PWYW 产品），添加到支付方法列表
	enableDodoTopUp := dodoPaymentsTopUpEnabled()
	if enableDodoTopUp {
		hasDodo := false
		for _, method := range payMethods {
			if method["type"] == "dodopayments" {
				hasDodo = true
				break
			}
		}
		if !hasDodo {
			dodoMethod := map[string]string{
				"name":      "信用卡 / 全球支付",
				"type":      "dodopayments",
				"color":     "rgba(var(--semi-violet-5), 1)",
				"min_topup": strconv.Itoa(setting.DodoPaymentsMinTopUp),
			}
			payMethods = append(payMethods, dodoMethod)
		}
	}
	...
	data := gin.H{
		...
		"enable_dodopayments_topup": enableDodoTopUp,
		"dodopayments_min_topup":    setting.DodoPaymentsMinTopUp,
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == "pending" {
			...
			model.MaybeGrantInviteRewardOnFirstTopUp(topUp.UserId)
		}
	} else {
