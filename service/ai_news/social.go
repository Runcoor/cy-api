...
字段约束:
- title: 中文,<= 25 个汉字,要勾人想点开,**禁止使用 emoji**
- body:
  - **必须是纯文本,不要任何 markdown 标记,禁止使用 emoji**
  - 不要 ##、**加粗**、- 列表、反引号代码、[]() 链接 —— 平台会原样显示成乱码
  - 段落用空行分隔
  - 想做小标题 → 用中文加冒号(例: "一句话总结：")
  - 想做要点列表 → 每行用「·」或「一、二、三」起手(例: "· 速度快")
  - 如果 kind = "image_only": 30-100 字,只是引子,价值在图里
  - 如果 kind = "text_image": 300-600 字
- tags: 3-6 个中文话题词,**不带 # 号,不要 emoji**,例 ["AI", "GPT", "OpenAI"]
- image_prompts:
  - **必须 2 或 3 张,不要更多**
